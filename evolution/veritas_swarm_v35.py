# -*- coding: utf-8 -*-
"""
VERITAS SWARM v3.5: "THE ARENA" (High-Performance Benchmark)
------------------------------------------------------------
...
class EpistemicPhysics:
    def __init__(self):
        # Słowa-klucze "Szumu Biurokratycznego" (High Entropy, Low Meaning)
    ...
    def measure(self, text: str) -> Dict[str, float]:
        ...
        # 1. Strukturalna Entropia (Kompresja)
        b_text = text.encode('utf-8')
        entropy = len(zlib.compress(b_text)) / len(b_text)
        
        # 2. Gęstość Semantyczna (Unikalne, znaczące tokeny)
        words = re.findall(r'\w+', text.lower())
        if not words: return {"score": 0, "density": 0, "entropy": 0}
        
        unique_meaningful = {w for w in words if w not in self.bureaucratic_vocab and len(w) > 3}
        density = len(unique_meaningful) / len(words)
        
        # 3. Indeks Biurokracji (Kara za szum)
        noise_count = sum(1 for w in words if w in self.bureaucratic_vocab)
        bureaucracy_penalty = noise_count * 0.1
        
        # FORMUŁA ADEPTHUSA (Calibrated v3.5)
        # Score = (Density * 4.0) - (Entropy * 1.2) - Penalty
        score = (density * 4.0) - (entropy * 1.2) - bureaucracy_penalty
        
        return {
            "score": score,
            "density": density,
            "entropy": entropy,
            "noise": bureaucracy_penalty
        }
